...
test("data bahan dapat diedit lengkap dan perubahan stok masuk riwayat opname", async () => {
  const [page, css] = await sources();

  assert.match(page, /Edit bahan/);
  assert.match(page, /modal==="stockedit"/);
  assert.match(page, /Simpan perubahan bahan/);
  assert.match(page, /Perubahan melalui Edit Bahan/);
  assert.match(css, /\.material-actions/);
  assert.match(css, /\.edit-material/);
});
